...
const FDIChartComponent = ({ countryName, countryCode, start = 2000, end = 2023, isDarkMode }) => {
  ...
  const fetchChart = async () => {
    ...
    try {
      const response = await fetch(`http://localhost:5000/api/fdi-chart/${countryCode}?start=${start}&end=${end}`);
      const data = await response.json();

      if (response.ok && data.imageBase64) {
        setImageBase64(data.imageBase64);
      } else {
        setError('FDI chart generation failed. Please try again.');
      }
    } catch (err) {
...
export default FDIChartComponent;
