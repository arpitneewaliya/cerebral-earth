...
const LiteracyChartComponent = ({ countryName, countryCode, start = 1980, end = 2023, isDarkMode }) => {
  ...
  const fetchChart = async () => {
    ...
    try {
      const response = await fetch(`http://localhost:5000/api/literacy-chart/${countryCode}?start=${start}&end=${end}`);
      const data = await response.json();

      if (response.ok && data.imageBase64) {
        setImageBase64(data.imageBase64);
      } else {
        setError('Literacy chart generation failed. Please try again.');
      }
    } catch (err) {
      ...
      {imageBase64 ? (
        <div className={`rounded-lg overflow-hidden border ${
          ...
          <img
            ...
            alt={`Literacy Chart of ${countryName}`}
...
export default LiteracyChartComponent;
